...
class NormalizedCategory(SqlAlchemyBase):
    ...
    dishes = orm.relationship("Dish", back_populates="normalized_category", cascade="all, delete-orphan")


@event.listens_for(NormalizedCategory, "before_delete")
def before_delete_normolize_category(mapper, connect, target, *my_own_paramters):
    base_category = connect.query(NormalizedCategory).get(1)
    for cat in target.categories:
        cat.category = base_category
    for dish in target.dishes:
        dish.normalized_category = base_category
    connect.commit()
